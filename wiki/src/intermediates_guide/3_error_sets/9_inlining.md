...
is very common when handling errors. And you may notice that the indentation level of the switch is 2 in our case. But because this pattern is so common, and because we do not further access `err` at all outside of the `switch` and we most likely do not write anything else in the catch scope other than the `switch`, we have developed a special case syntax (sugar) for this case:
...
will not compile. Because now we switch on the error which is of type `anyerror`, and we **cannot** switch on the `anyerror` type. So, inline-switching is only possible on functions which explicitely name their potential errors they might throw.
