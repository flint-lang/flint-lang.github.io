...
This feature, while *defintely* planned, is not implemented yet. This feature becomes *extremely* useful in the context of function composition and blueprints, so it's considered a "must-have" feature for Flint. Inferring the error set has a low syntax surface but has **huge** compiler requirements, which the compiler just does not support yet to enable them.
