...
First, lets recap the structure of optionals for a second. An optional `i32?` looks like this in memory:
...
Okay and now lets stop acting like padding and alignment are nonexistent. Because the largest alignment of all types we need to be able to store is `8` (from the `i64`) there are `7` padding bytes inserted into the structure. So the above variant will *not* be only `9` bytes in memory but it will be `16` bytes instead. I will not go into detail about padding and alignment here, if you are unfamiliar with it I recommend you looking it up, it helps you understand how memory works.

Okay and now lets talk about the `type` flag, because it's pretty interesting as well... When we define our variant from left to right we assign type IDs to each possible type of our variant. In our case the type `i32` will have the ID of `1`, `f32` will have `2` and `u64` will have `3`. But where is the `0`, you may ask. This will be explained in a later chapter when talking about optional variants, but for now just know that the value `0` is reserved for the absence of values (optionals).
