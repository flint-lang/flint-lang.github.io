...
So, this mutability allows us to circumvent Flint's safety mechanisms around accessing the value field of an empty optional variable, which is rather interesting:
...
So, you are not allowed to access the value when there is nothing stored on it but if there *was* something stored on it and you *still* have the reference to the value field you can actually still modify it. Maybe we will add a system to detect and prevent this behaviour (for example by marking the switched-on variable as immutable inside the value branch, that would be rather easy), but it's very funny in my opinion that you can do it at all.
