...
Lets discuss what happens here under the hood within the compiler and within Flint itself. You already know what a `struct` is in other languages, and in Flint that's either an anonymous tuple or `data`. So, an optional is essentially nothing else than a small struct containing of a `bool` value together with the *actual* value of the optional. In the case of our optional, this means that `maybe` is essentially the same as if we would write:
