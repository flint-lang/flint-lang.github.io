...
We now know how to access the value of an optional variable, but how do we check if the variable holds a valid value at all? **Any** optional value can bo compared with the `none` literal, so we can compare for equality / inequality very easily. Here is an example of that:
