...
There is a lot to unpack here. You can safely ignore the `use Core.print` line for now. Just remember that this line needs to be written in order for the `print` function to be accessible. But what that line actually does will be described in a [later](../5_functions.md) chapter.

The `def main():` line is the definition of the main function, but functions are described in a [later](../5_functions.md) chapter too.
