...
I know Flint is technically still OOP (when thinking about modern definitions of it), but I use OOP as a term to describe inheritance-based designs in general here.

In an OOP-world, where classes inherit other classes you would get a **identity hierarchy** tree. In this world view you are always asking what something *is*. *Is* this class, like `Dog` an `Animal`? OOP is filled with the question about identity, whether something *is* a specialization of something more general.

This question about identity helps in modelling *type relationships* of classes but it comes with a cost, both mentally and in the runtime. Casting specialized objects to more generous ones, the potential of `null`ability if casting fails etc. In the world of composition you do not ask yourself what something *is* but rather what something *has*. You do not care about it's **identity**, you only care about it's **capabilities**. For example, in the following example about animals, which is a *very* common early-teaching OOP example, we would model it very differently in Flint.
...
> ```
> Starting to run with my 4 legs with a speed of 30.0 km/h
...
Also, the composition-based approach is not new nor is Flint the only language where you can write composition-based code. There are a lot of languages where you can write it, those are often "more normal" OOP languages too, but in Flint you are *required* to model your world in a compositional way, which could make it a great opportunity for you to learn that mindset, and potentially apply it to other languages too.
...
def main():
	o := Object{
		Transform{i32x2{100, 120}, i32x2{200, 220}},
		Scale{f32x2{3.3, 4.4}},
		};
	o.print();
	print($"o.pos = {o.get_pos()}\n");
