...
Lets finally come to tags. They are absoltely essential for Flint's interop capabilities, and they once again would not have been possible without the FIP.

Lets say we have a `extern.c` file like this:
...
The *module tag* is the `[c]` in our case. We could write *any* text in there. The text should be an identifier, however, so it should only contain alphanumerics and underscores, just like defining any variable or type name in Flint.
...
This means that now *all* things coming from C need to be prefixed with a `c.` which is pretty convenient. It also means that there are no naming collisions possible between symbols coming from C and those coming from your own codebase.

## Multiple Tags

As you might have guessed already, `fip-c` is not only limited to one tag either. So, lets add a second file to the `extern.c`, let's call it `ex2.c` and it should look like this:
...
And then we just modify the `fip-c.toml` configuration file to look like this:
...
Lets look at the `ex2.ft` which was generated from the `ex2.c` file:
...
And boom there now no longer exist any collisions between those two libraries (if there even were any) and you can use them both in Flint.
