...
Enhanced for loops are for loops without explicitely declaring a range like in regular for loops, they instead directly operate on a so-called **iterable**. They are extremely useful for iterating through arrays, as enhanced for loops will **always** iterate through a multidimensional array *sequentially*.
...
As you can see, modifying `elem` directly modifies the array at the current index inplace. This is very powerful for mutli-dimensional arrays, because yes, multidimensional arrays are considered iterables too!

## Not using index or elem

We can opt out of using the `index` or `elem` variables for enhanced for loops entirely through the `_` operator. Again, it is used in the context of `unused / discarded` here. Here is a small example:
...
If you need access to all current coordinates when iterating through multi-dimensional arrays you need to fall back to using regular for loops again, as we did above.

## Iteration Context

The group with the index and the element is called the **iteration context** but what does this mean and why does it have a name? Its actually pretty simple: Because this context can be a tuple variable as well!
